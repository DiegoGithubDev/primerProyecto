...
class Estudiante extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fkUsuario', 'numerical', 'integerOnly'=>true),
			array('nombre, apellido', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre, apellido, fkUsuario', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'boletins' => array(self::HAS_MANY, 'Boletin', 'fkEstudiante'),
			'fkUsuario0' => array(self::BELONGS_TO, 'Usuario', 'fkUsuario'),
			'inscripcions' => array(self::HAS_MANY, 'Inscripcion', 'fkEstudiante'),
		);
	}
